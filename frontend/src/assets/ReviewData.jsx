export const reviews = [
    {
      Reviewid: 1,
      image:
        'https://images.unsplash.com/photo-1605462863863-10d9e47e15ee?q=80&w=2070&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D',
      name: 'John Doe',
      text: "This book completely captivated me from start to finish. The characters are deeply relatable, and the plot twists kept me on the edge of my seat. A must-read for anyone who loves suspense and complex storytelling!",
      Rating: 2
    },
    {
      Reviewid: 2,
      image:
        'https://images.unsplash.com/photo-1438761681033-6461ffad8d80?q=80&w=2070&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D',
      name: 'Jane Smith',
      text:  "I enjoyed the vivid descriptions and the author's ability to create a rich, immersive world. While the pacing was a bit slow at times, the depth of the characters and the philosophical themes explored made it a rewarding read."
      ,
      Rating :3
    },
    {
      Reviewid: 3,
      image:
        'https://images.unsplash.com/photo-1463453091185-61582044d556?q=80&w=2070&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D',
      name: 'Alex Johnson',
      text: "The premise of the book was intriguing, but I found the execution somewhat predictable. It's a decent read for a lazy weekend, but I wished for more depth in character development and a less formulaic plot.",
      Rating :5
    },
    {
      Reviewid: 4,
      image:
        'https://plus.unsplash.com/premium_photo-1671823917954-dc943c1bd9df?q=80&w=2070&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D',
      name: 'Emily Davis',
      text: "This book is a masterpiece of storytelling. The author's prose is lyrical and evocative, drawing you effortlessly into the lives of the characters. Themes of love, loss, and redemption are delicately woven together, leaving a profound impact. A deeply moving and unforgettable read that lingers in your thoughts long after you turn the final page."





,
      Rating :4
    },
    {
      Reviewid: 5,
      image:
        'https://images.unsplash.com/photo-1564564321837-a57b7070ac4f?q=80&w=1776&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D',
      name: 'David Miller',
      text: "The product not only met but exceeded my expectations. It's clear that the team is dedicated to delivering high-quality work. I'm a satisfied customer.",
      Rating : 4
    },
  ];