...
function RemoveBook() {
  ...
  const changeSortField = (field) => {
    ...
    const sortedBooks = !result
      ? sortFun(posts, sortBy, field)
      : sortFun(result, sortBy, field);
    setBook(sortedBooks);
  };
  ...
  return (
    <>
      ...
      <div className="mb-8">
        <form
          className="flex flex-row gap-4 py-20 mb-6 mx-w-l m-auto  justify-center"
          style={{
        ...
        >
          ...
          <div className="mb-4">
            <span className="block text-white text-lg font-bold mb-2">
              Sort Field:
            </span>
            <select
              name="field"
          ...
          {user && user.role === "admin" && (
            <div className="flex items-center">
              <a
                href="/books/add"
                className="mt-2 px-4 py-2 bg-green-700 text-white rounded-lg hover:bg-green-800 hover:scale-105"
              >
                Add New
              </a>
            </div>
        ...
        <div className="mx-auto px-[15%] min-h-24">
          <div className="grid grid-cols-1 gap-4">
            {books.map((book) => (
              <div
                className="flex py-2 px-4 border border-gray-300 rounded-3xl shadow-md bg-primary/20 gap-2"
                key={book.Title_name}
              >
                <div className="w-[10%] mr-6">
                  <img
                    src={`${book.Img_url}`}
                    alt="pic"
                    className="rounded-sm w-full  mx-auto my-auto"
                  />
                </div>
                <div className="w-[70%] flex flex-col items-start">
                  ...
                  <Link
                    to={`/books/details/${book.Title_ID}`}
                    className="text-xs mt-2 px-2 py-1 bg-secondary text-white rounded-lg hover:bg-secondary/90 hover:scale-105"
                  >
                    more details
                  </Link>
                </div>
                {user && user.role === "admin" && (
                  <>
                    <div className="flex items-center">
                      <Link
                        to={`/books/edit/${book.Title_ID}`}
                        className="text-xs mt-2 px-4 py-2 bg-green-700 text-white rounded-lg hover:bg-green-800 hover:scale-105"
                      >
                        Update
                      </Link>
                    </div>
                    <div className="flex items-center">
                      <button
                        onClick={() => handleDeleteClick(book.Title_ID)}
                        className="text-xs mt-2 px-4 py-2 bg-red-700 text-white rounded-lg hover:bg-red-800 hover:scale-105"
                      >
                        Delete
                      </button>
                    </div>
            ...
            {books.length === 0 && (
              <h2 className="text-center text-red-600">Empty List!</h2>
            )}
          </div>
        </div>
      </div>
