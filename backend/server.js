...
app.get("/book/:id", (req, res) => {
  const sql =
    "SELECT * FROM `book title` INNER JOIN author ON `book title`.Author_ID = author.Author_ID WHERE `book title`.Title_ID = ?";
  const id = req.params.id;
...
app.post("/issue", (req, res) => {
  ...
  const sql =
    "INSERT INTO issuebook (`Admin_ID`, `Member_ID`, `Book_ID`, `Issued_Date`) VALUES (?, ?, ?, ?)";
...
app.post("/returnbook", (req, res) => {
  ...
  //console.log(id, date)
  const sql =
    "UPDATE issuebook SET `Returned_Date` = ? WHERE `Issue_ID` = ? AND `Returned_Date` IS NULL";
